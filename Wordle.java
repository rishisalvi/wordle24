 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Rishi Salvi
 *	@version	1.0
 *	@since		10/16/23
 */ 
public class Wordle
{ 
	 ...
	 *	@return					the word chosen as tzebrahe "goal word".
	 *	THIS METHOD IS INCOMPLETE.
	 */
	public String openFileAndChooseWord(String inFileName, String testWord)
	{
		...
		int numWords = 0; 
		Scanner scan = FileUtils.openToRead(inFileName); 
		while(scan.hasNext()){
			numWords++;
			validWords.add(scan.nextLine());
		}
		
		int chooseWord = (int)(Math.random() * (numWords + 1)); // makes a random int from 0 to numWords
		result = validWords.get(chooseWord); 
	...
	{
		Scanner scan = FileUtils.openToRead("words5allowed.txt"); 
		while (scan.hasNext()){
			String current = scan.nextLine();
			if (possibleWord.equalsIgnoreCase(current))
				return true; 
		}
		return false;
	}
	...
	public void drawPanel ( )
	{
		...
		int[][] letterGuess = new int[6][5]; 
		for (int a = 0; a < 6; a++){
			for (int b = 0; b < 5; b++)
				letterGuess[a][b] = 0; 
		}
		for (int i = 0; i < 6; i++){
			String current = wordGuess[i].toLowerCase(); 
			for (int j = 0; j < current.length(); j++){
				char index = current.charAt(j); 
				if (index == word.charAt(j)){
					letterGuess[i][j] = 3; 
					current = current.substring(0, j) + " " + current.substring(j + 1); 
				}
				else if (word.indexOf(index) > -1){
					letterGuess[i][j] = 2; 
					current = current.substring(0, word.indexOf(index)) + " " + current.substring(word.indexOf(index) + 1); 
				}
				else{
					letterGuess[i][j] = 1; 
				}
		...
		for(int [] pair : Constants.KEYPLACEMENT)
		{
			...
			{
				char c = Constants.KEYBOARD[place].toLowercase(); 
				for (int i = 0; i < 6; i++)
